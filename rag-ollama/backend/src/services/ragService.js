...
const generateResponse = async (query, documents) => {
  try {
    ...
    // If no relevant documents found, just pass the query to the model
    ...
    const context = relevantDocs.map(doc => `Document Title: ${doc.title}\nContent: ${doc.content}`).join('\n\n');
...
Question: ${query}`;
    ...
    return {
      answer: response.message.content,
      documents: relevantDocs.map(doc => ({ id: doc.id, title: doc.title }))
    };
  } catch (error) {
...
Question: ${query}`;
    ...
    return relevantDocs.map(doc => ({ id: doc.id, title: doc.title }));
  } catch (error) {
